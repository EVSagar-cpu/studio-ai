...
/* ════════════════════════════════════════════════════════════════════════
   VIEW: OUTPUT LIBRARY
════════════════════════════════════════════════════════════════════════ */
function OutputView({ T, mode, assets }) {
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          <button onClick={() => setFilter("all")} style={{
            ...
            fontSize: 12, fontFamily: "inherit", fontWeight: filter === "all" ? 600 : 400,
            background: filter === "all" ? T.acd : T.s2, color: filter === "all" ? T.ac : T.t2
          }}>All ({approved.length})</button>
          {subjects.map(s => (
            <button key={s} onClick={() => setFilter(s)} style={{
              padding: "4px 10px", borderRadius: 6, border: "none", cursor: "pointer",
              fontSize: 12, fontFamily: "inherit", fontWeight: filter === s ? 600 : 400,
              background: filter === s ? T.acd : T.s2, color: filter === s ? T.ac : T.t2
            }}>{s}</button>
          ))}
        </div>
      ...
      {shown.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: T.t3 }}>
          <CheckCircle size={32} style={{ margin: "0 auto 12px", display: "block", opacity: .25 }} />
          <div style={{ fontSize: 14, color: T.t2 }}>No approved videos yet</div>
          <div style={{ fontSize: 12, marginTop: 4 }}>Mark assets as Approved in the Review Queue</div>
        </div>
      ) : viewMode === "grid" ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 14 }}>
          {shown.map(a => (
            <Card key={a.id} T={T} style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ height: 160, background: "#0a0a0c", position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(99,102,241,.3)", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)", cursor: "pointer" }}>
                  <Play size={18} color="#fff" style={{ marginLeft: 2 }} />
                </div>
                <div style={{ position: "absolute", top: 10, left: 10 }}><Badge status={a.status} mode="dark" /></div>
                <div style={{ position: "absolute", top: 10, right: 10 }}><ScoreDot val={a.score} T={DARK} /></div>
                <div style={{ position: "absolute", bottom: 10, right: 10, background: "rgba(0,0,0,.6)", padding: "2px 7px", borderRadius: 4, fontSize: 11, color: "#fff", fontFamily: "monospace" }}>{a.duration}</div>
              </div>
              <div style={{ padding: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: T.t, marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.title}</div>
                <div style={{ fontSize: 11, color: T.t3, marginBottom: 10 }}>{a.subject} · Grade {a.grade}</div>
                <div style={{ display: "flex", gap: 6 }}>
        ...
        <Card T={T} style={{ padding: 0, overflow: "hidden" }}>
          {shown.map((a, i) => (
            <div key={a.id} style={{
              ...
              borderBottom: i < shown.length - 1 ? `1px solid ${T.b}` : "none"
            }}>
              <div style={{ width: 44, height: 44, borderRadius: 8, background: T.s2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer" }}>
                <Play size={16} color={T.t3} style={{ marginLeft: 2 }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: T.t, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.title}</div>
                <div style={{ fontSize: 11, color: T.t3 }}>{a.subject} · Grade {a.grade} · {a.duration} · {a.date}</div>
              </div>
              <ScoreDot val={a.score} T={T} />
              <Badge status={a.status} mode={mode} />
              <div style={{ display: "flex", gap: 4 }}>
                <button style={{ padding: 5, border: "none", background: "transparent", cursor: "pointer", color: T.t3, borderRadius: 5 }}><Eye size={14} /></button>
                <button style={{ padding: 5, border: "none", background: "transparent", cursor: "pointer", color: T.t3, borderRadius: 5 }}><Download size={14} /></button>
                <button style={{ padding: 5, border: "none", background: "transparent", cursor: "pointer", color: T.t3, borderRadius: 5 }}><Pencil size={14} /></button>
              </div>
            </div>
          ))}
        </Card>
